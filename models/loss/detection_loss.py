...
class DetectionLoss(tf.keras.losses.Loss):
    ...
    def get_grid_shapes(self, detections):
        # return [
        #    (detection.shape[1], detection.shape[2]) for detection in detections
        # ], detections[0].shape[0]
        return (detections.shape[1], detections.shape[2]), detections.shape[0]

    def get_grid_offsets(self, grid_x_num, grid_y_num, batch_size, anchor_size):
        """"""
        intermediate_center_x_offset = tf.broadcast_to(
            tf.range(grid_x_num, dtype=tf.float32), [grid_y_num, grid_x_num]
        )
    ...
    def get_anchor_wh_offsets(
        self, w_index, h_index, anchors, grid_x, grid_y, batch_size
    ):
        anchor_size = anchors.shape[0]
        w_offsets = tf.broadcast_to(
            tf.expand_dims(
    ...
    def get_localization_loss(self, loss_map, true, pred):
        return loss_map["mse"](true, pred)

    def get_noobj_loss(self, loss_map, true, pred):
        return loss_map["bce"](true, pred)

    def get_obj_loss(self, loss_map, true, pred):
        return loss_map["bce"](true, pred)

    def get_class_loss(self, loss_map, true, pred):
        return loss_map["bce"](true, pred)
    ...
    def get_loss_per_grid_detection(
        ...
        w_index = 0
        h_index = 1
        w_offsets, h_offsets = self.get_anchor_wh_offsets(
            w_index, h_index, anchors_tensor, grid_x, grid_y, batch_size
        )
        ...
        # get the converted gt and the matched priors from predictions
        priors_from_preds = tf.gather_nd(output_bboxes, prior_indices)
        converted_gt = self.convert_gt_for_loss(
            ground_truth_bbox,
            priors_x,
            priors_y,
            matched_anchors,
        ...
        # at this step, you can get localization loss from priors_from_preds
        # and converted_gt
        localization_loss = self.get_localization_loss(
            loss_map, converted_gt, priors_from_preds
        )
        ...
        obj_loss = self.get_noobj_loss(
            loss_map, objects_positives_gt, objects_positives_preds
        )
        noobj_loss = self.get_obj_loss(
            loss_map, objects_negative_gt, objects_negative_preds
        )
        ...
        # get bce loss for classes
        class_loss = self.get_class_loss(loss_map, class_gt, class_predictions)
    ...
    def __call__(self, y_true, y_pred, index_to_use, sample_weight=None):
        # i.e y_true = ground_truth
        #     y_pred = detection_outputs
        ground_truth = y_true
        detection_outputs = y_pred
        grid_shapes, batch_size = self.get_grid_shapes(detection_outputs)
        self.batch_size = batch_size
        # detection_losses = []

        # for detection_index, detection_output in enumerate(detection_outputs):
        #    current_detection_loss = self.get_loss_per_grid_detection(
        #        detection_output,
        #        self.current_loss_map,
        #        ground_truth,
        #        self.anchors_list[detection_index],
        #        grid_shapes[detection_index],
        ...
        #    )
        #    detection_losses.append(current_detection_loss)
        # self.loss_list = detection_losses

        # total_loss = 0.0
        # for detection_loss in detection_losses:
        #    total_loss += detection_loss["total_loss"]

        total_loss = self.get_loss_per_grid_detection(
            detection_outputs,
            self.current_loss_map,
            ground_truth,
            self.anchors_list[index_to_use],
            grid_shapes,
            (image_width, image_height),
            batch_size,
            num_classes,
