...
def parse_convolution_proto(cfg):
    ...
    return ConvolutionLayer.ConvolutionLayer(
...
def parse_residual_proto(cfg):
    ...
    for _ in range(residual_multiplier):
        residual_block_contents.append(
            [parse_convolution_proto(conv_cfg) for conv_cfg in cfg.bare_convs]
        )
    return ResidualBlock.ResidualBlock(
        block_contents=residual_block_contents, multiplier=residual_multiplier
    )
...
def parse_route_proto(cfg):
    ...
    return RouteLayer.RouteLayer(indices=input_indices)


def parse_upsample_proto(cfg):
    upsampling_factor = cfg.factor
    return UpSamplingLayer.UpSamplingLayer(factor=upsampling_factor)


def parse_yolo_proto(cfg):
    anchors_cfg = list(cfg.anchors)
    len_anchors_cfg = len(anchors_cfg)
    assert len_anchors_cfg % 2 == 0, "Odd number ofarguments {}".format(len_anchors_cfg)
