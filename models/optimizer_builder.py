...
def build(config):
    ...
    optimizer_args = {
        ...
        for field in optimizer_pb.DESCRIPTOR.fields
    }
    ...
    if learning_rate_string != "constant":
        ...
        learning_rate_args = {
            field.name: getattr(learning_rate_pb, field.name)
            for field in learning_rate_pb.DESCRIPTOR.fields
        }
    else:
        learning_rate_pb = getattr(config, learning_rate_string)
    ...
    learning_rate_scheduler = learning_rate_scheduler_map[learning_rate_string](
        **learning_rate_args
    )
